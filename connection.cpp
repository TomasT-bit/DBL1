...
std::mutex neo4j_mutex;

json post_query(const string& query, const json& params = json::object()) {
    std::lock_guard<std::mutex> lock(neo4j_mutex);
    std::cout << "Running query:\n" << query << "\n";
    if (!params.empty()) std::cout << "With parameters:\n" << params.dump(2) << "\n";

    json payload = {
        {"statements", {{{"statement", query}, {"parameters", params}}}}
    };

    cpr::Url url("http://localhost:7474/db/twitter9/tx/commit");
    cpr::Authentication auth("neo4j", "password", cpr::AuthMode::BASIC);
    cpr::Header headers = {{"Content-Type", "application/json"}};
    cpr::Body body(payload.dump());

    auto r = cpr::Post(url, auth, headers, body);
...
void handle_airline(const string& airline_id, int& global_conv_id) {
    ...
    for (const auto& [comp_id, children_map] : all_components) {
        ...
        bool has_airline = any_of(trimmed.begin(), trimmed.end(), [&](const string& t) {
        ...
        json conv_params = {
            {"convId", global_conv_id},
            {"airlineId", airline_id}
        };
        post_query("MERGE (c:Conversation {id: $convId}) SET c.airlineId = $airlineId", conv_params);

        for (const auto& tid : trimmed) {
            json edge_params = {
                {"tweetId", tid},
                {"convId", global_conv_id},
                {"positionType", annotations[tid]}
            };
            post_query(R"(
                MATCH (t:Tweet {tweetId: $tweetId})
                MERGE (c:Conversation {id: $convId})
                MERGE (c)-[:PART_OF {positionType: $positionType}]->(t)
            )", edge_params);
        }
...
/* cl /std:c++17 connection.cpp /EHsc ^
